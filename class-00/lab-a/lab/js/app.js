...
function displayPics(){
  ...
  // It said that "rando" was not defined. Because rando is a local variable in the displayPics func not a global.
  console.log(viewed);
...
function handleClick(event) {
  ...
  for(let i = 0; i < names.length; i++){
    if(event.target.id === allProducts[i].name) {
      ...
      console.log(`${event.target.id} has ${allProducts[i].votes} votes and ${allProducts[i].views} views`);
    }
  }
...
function showList() {
  for(let i = 0; i < allProducts.length; i++) {
    ...
    liEl.textContent = `${allProducts[i].name} has ${allProducts[i].votes} votes and ${allProducts[i].views} views`;
